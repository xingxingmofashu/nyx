...
export class Tasks {
  static register(server: NyxServer): void {
    Guard.handle(IPC.tasks.imageToImage.run, (_e, modelId: string, input: ImageBytes) =>
      server.imageToImage(modelId, input),
    )
    Guard.handle(IPC.tasks.textToSpeech.run, (_e, modelId: string, input: TextToSpeechInput) =>
      server.textToSpeech(modelId, input),
    )
    Guard.handle(IPC.tasks.automaticSpeechRecognition.run, (_e, modelId: string, input: AudioSamples) =>
