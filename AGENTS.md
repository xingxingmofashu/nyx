...
- `packages/server` is a single `Server` namespace (`Server.App`, `Server.Errors`, the middleware under `Server.Middleware.Auth`, and the Hono sub-app classes under `Server.Routes.*`); `Server.App.create({ token, onLog })` builds the services from `@nyx/agent` and mounts each `Server.Routes.*` under `/v1`, while `Server.App.serve` starts the node server (`src/bin.ts` is the `nyx-server` binary entry, which reads the env and dynamically imports `src/index.ts` after setting `HF_ENDPOINT`). Request bodies are validated with `@hono/zod-validator` (`Server.Errors.hook`); routes throw `Server.Errors.status(...)`/`HTTPException` and a single `App` `onError` renders `{ error }` with the exception's status (500 otherwise). `Server.AppType` is exposed for `hc<Server.AppType>` on the desktop side; the wire schemas/types live in the `Agent` namespace (`Agent.SessionSaveRequestSchema`, `Agent.KnowledgeStatus`, …) and the desktop imports them type-only from `@nyx/agent`.
...
- Agent settings live only in `settings.json` (no env overrides). Remaining env vars: `NYX_WEB_SEARCH_PROVIDER/API_KEY`, `NYX_PARALLEL_API_KEY`, `HF_ENDPOINT` (HF mirror), `NYX_MODELS_DIR`, `NYX_KNOWLEDGE_DIR`, `NYX_SERVER_TOKEN/PORT/HOST`.
