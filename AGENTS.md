...
- `packages/server` — Hono app on port 3848 (`127.0.0.1`), routes `/api/health`, `/api/chat`. Constructed with an `Agent`.
- `apps/cli` — yargs CLI (`nyx text-generation`, `nyx image-to-image`, `nyx server`, `nyx tui`). `src/cli/utils/engine.ts` wires llm/core/server together; `src/cli/utils/cmd.ts` is the yargs typing helper.
