...
- Zod is the single source of truth: export the schema as the value `XSchema` and its inferred type as `type XSchemaType = z.infer<typeof XSchema>` (`SettingsSchema`/`SettingsSchemaType`, `ModelsSchema`/`ModelsSchemaType`). No hand-written duplicate types for schema-backed data.
...
- `src/index.ts` is the only public entry and exports exactly one `export namespace Global`. Each `src/<concept>.ts` is pulled in as `import * as xModule from "./x.ts"` and re-exported as `export import X = xModule.X` (values: classes, schemas) or `export type T = xModule.T` (types). Consumers write `import { Global } from "@nyx/global"`, never `import * as`.
- Directory layout lives on `Path` (static getters, reads `NYX_*` env) and `Workspace` (sessions/audio/images/attachments dirs). `Session`'s public types (`ChatSessionMeta`/`ChatSession`/`SessionSaveInput`) are the only hand-written ones, with its stored-meta schema kept private.
- Bun-only and self-contained (runtime deps: `defu`, `fs-extra`, `zod`); must not pull in ONNX/transformers/LanceDB, and is never runtime-imported by the desktop. `@nyx/agent/schema` re-aliases `Global.*SchemaType` to the wire names (`Settings`, `AgentSettings`, `ModelInfo`, …), so the HTTP API and the desktop never depend on global's internal type names.
