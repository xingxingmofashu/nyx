...
const TASK_CHOICES = ["image-to-image", "text-to-audio"] as const;
...
export const PullCommand = cmd<Record<string, unknown>, PullArgs>({
  ...
  builder: (yargs) =>
    yargs
      ...
      .option("task", {
        ...
        demandOption: true,
        description: "Pipeline task: image-to-image or text-to-audio",
      })
      .option("dtype", {
        type: "string",
  ...
  handler: async (args: PullArgs) => {
    if (!args.model) {
      log.error("Usage: nyx model pull <model> --task <image-to-image|text-to-audio> [--dtype <dtype>]");
      process.exit(1);
    }
