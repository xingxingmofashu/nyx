...
- Desktop dev needs the server bundle first: `bun --cwd packages/server run build`; NyxServer resolves `packages/server/dist/server.cjs` (dev) or a packaged `resources/server.cjs`.
